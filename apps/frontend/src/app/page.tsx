      ...
      <div className="relative overflow-hidden rounded-2xl border border-red-200 bg-gradient-to-r from-red-100 via-white to-red-50 p-6 md:p-8 shadow-sm dark:border-red-900/40 dark:bg-gradient-to-r dark:from-red-950/80 dark:via-slate-900 dark:to-slate-950 dark:shadow-xl transition-colors duration-200">
        <div className="relative z-10 max-w-3xl space-y-3">
          ...
          <p className="text-sm md:text-base text-slate-600 dark:text-slate-300">
            Managing hierarchical sports associations, multi-tier player and official licenses,
            Davis-Cup style league encounters, tournaments, and real-time live scoring.
          </p>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 transition-colors duration-200">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Associations</span>
            <Network className="h-5 w-5 text-red-500" />
          </div>
          <div className="mt-3 text-2xl font-bold text-slate-900 dark:text-white">{associations.length || 4}</div>
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">National & Regional DAG Tree</p>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 transition-colors duration-200">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Affiliated Clubs</span>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 transition-colors duration-200">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Active Licenses</span>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 transition-colors duration-200">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Competitions</span>
            <Trophy className="h-5 w-5 text-cyan-500" />
          </div>
          <div className="mt-3 text-2xl font-bold text-slate-900 dark:text-white">{competitions.length || 2}</div>
                ...
                <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-600 dark:text-slate-400 line-clamp-1">
                      {comp.description || 'Official championship with group rounds and playoff encounters.'}
                    </p>
                  </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  National rules overrule regional sub-association rules. Sub-associations can belong to multiple parent federations.
                </p>
              </div>
              <Link
                href="/associations"
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            {calendarEvents.slice(0, 5).map((evt) => (
              ...
              >
                ...
                <h4 className="mt-2 text-sm font-semibold text-slate-800 dark:text-slate-200 line-clamp-1">{evt.title}</h4>
                {evt.location && (
          ...
          <div className="rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/40 p-4 shadow-sm">
            ...
            <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
              National pattern: <code className="text-red-600 dark:text-red-400 bg-slate-100 dark:bg-slate-950 px-1 py-0.5 rounded font-mono">{nationalAssoc?.licenseIdTemplate || '{regionDigit}{year2}{counter3}'}</code>
            </p>
            <div className="mt-3">
              <Link
