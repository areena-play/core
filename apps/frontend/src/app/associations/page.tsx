...
export default function AssociationsPage() {
  ...
  useEffect(() => {
    if (selectedAssoc) {
      api.getAssociationRules(selectedAssoc.id).then(setEffectiveRules).catch(() => {});
    }
  }, [selectedAssoc]);
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Multi-parent Directed Acyclic Graph (DAG) association hierarchy with national rule precedence and club affiliations.
          </p>
        </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-xs text-slate-400 font-bold">[{nat.code}]</span>
                    </div>
                  ...
                  <div className="text-xs text-slate-400 font-mono">
                    Template: <span className="text-red-400 font-bold">{nat.licenseIdTemplate}</span>
                  </div>
                </div>
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-mono text-xs text-red-400 font-bold">[{sub.code}]</span>
                        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Shows active rules, with national rules overriding local rules according to federation charter.
              </p>
            </div>
          ...
          {hierarchy.clubs?.map((club: any) => (
            ...
            >
              ...
              <div className="pt-2 border-t border-slate-800/60">
                <span className="text-[10px] font-semibold text-slate-500 uppercase">Affiliated With:</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {club.associations?.map((ca: any) => (
                    <span key={ca.association.id} className="rounded bg-slate-800 px-1.5 py-0.5 text-[10px] text-slate-300">
                      {ca.association.name}
                    </span>
                  ))}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setShowClubModal(false)} className="text-slate-400 hover:text-white">✕</button>
            </div>
            ...
            <form onSubmit={handleCreateClub} className="space-y-3 text-xs">
              ...
              <div>
                <label className="font-semibold text-slate-300">Parent Sub-Associations (Multi-Select DAG)</label>
                <div className="mt-1 max-h-32 overflow-y-auto rounded-lg border border-slate-800 bg-slate-950 p-2 space-y-1">
                  {hierarchy.associations?.map((a: any) => (
                    <label key={a.id} className="flex items-center gap-2 cursor-pointer text-slate-300">
                      <input
                        type="checkbox"
                        checked={selectedAssocIds.includes(a.id)}
                        onChange={(e) => {
                          if (e.target.checked) setSelectedAssocIds([...selectedAssocIds, a.id]);
                          else setSelectedAssocIds(selectedAssocIds.filter((id) => id !== a.id));
                        }}
