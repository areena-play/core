  ...
  const fetchPending = async () => {
    try {
      ...
      const filtered = data.filter((l: any) =>
        l.status === 'PENDING_CLUB' || l.status === 'PENDING_ASSOCIATION'
      );
    ...
    <div className="space-y-6 pb-12">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Club and Association administrative approval portal for submitted player, coach, and referee license requests.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-12 text-center text-slate-400 space-y-2">
          ...
          <p className="text-xs">There are no pending license applications requiring your administrative review.</p>
        </div>
      ) : (
            ...
            >
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-4 text-xs text-slate-400">
                  <span><strong>Association:</strong> {lic.association?.name}</span>
                  {lic.club && <span><strong>Club:</strong> {lic.club.name}</span>}
                  <span><strong>Applied on:</strong> {format(new Date(lic.createdAt), 'PPP')}</span>
                </div>
              </div>
