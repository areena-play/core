  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.autoApproved) {
        setSuccessMsg('🎉 T-Card License has been AUTO-APPROVED based on domestic country criteria! Your license is active immediately.');
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 md:p-8 shadow-xl space-y-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Choose your role and license category. Roles grant official eligibility for league encounters, ranked tournaments, or coaching & refereeing.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          {(user?.isSuperAdmin || (user?.clubRoles && user.clubRoles.length > 0)) && (
            <div>
              ...
              >
                <option value="">Apply for Myself ({user?.firstName} {user?.lastName})</option>
                {allUsers.map((u) => (
                  <option key={u.id} value={u.id}>
                    {u.firstName} {u.lastName} ({u.email}) {u.licenseId ? `- ID: ${u.licenseId}` : ''}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                ...
                <div className="font-bold text-white flex items-center justify-between">
                  ...
                  <span className="text-[10px] rounded bg-red-600/30 text-red-400 px-1.5 py-0.5">Club Attached</span>
                </div>
                <p className="mt-2 text-[11px] text-slate-400">
                  Strictly 1 regular license per player per season. Valid for all national and regional league encounters.
                </p>
              </label>
              ...
              >
                ...
                <div className="font-bold text-white flex items-center justify-between">
                  ...
                  <span className="text-[10px] rounded bg-emerald-600/30 text-emerald-400 px-1.5 py-0.5">Auto-Approval</span>
                </div>
                <p className="mt-2 text-[11px] text-slate-400">
                  For ranked tournament participation. No club attachment required. Instant auto-approval for domestic players.
                </p>
              </label>
              ...
              >
                ...
                <div className="font-bold text-white flex items-center justify-between">
                  ...
                  <span className="text-[10px] rounded bg-purple-600/30 text-purple-400 px-1.5 py-0.5">Refresher Required</span>
                </div>
                <p className="mt-2 text-[11px] text-slate-400">
                  Accredited coaching certification. Requires periodic refresher courses attested by federation instructors.
                </p>
              </label>
              ...
              >
                ...
                <div className="font-bold text-white flex items-center justify-between">
                  ...
                  <span className="text-[10px] rounded bg-blue-600/30 text-blue-400 px-1.5 py-0.5">Refresher Required</span>
                </div>
                <p className="mt-2 text-[11px] text-slate-400">
                  Federation match referee credential. Requires attendance at annual rule update refresher seminars.
                </p>
              </label>
            </div>
          </div>
