...
export default function RefresherCoursesPage() {
  ...
  const handleAttest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAttestSuccess('Attendance successfully attested! Member license validity extended by ' + selectedCourse.validityExtensionMonths + ' months.');
      fetchCourses();
      setTimeout(() => {
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Mandatory continuing education seminars. Federation instructors attest attendance to extend license validity periods.
          </p>
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5 text-xs text-slate-400 border-t border-slate-800 pt-3">
                <div className="flex items-center gap-2">
                  ...
                  <span>{format(new Date(c.date), 'PPPP p')} ({c.durationHours} hrs)</span>
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>Instructor: {c.instructor?.firstName} {c.instructor?.lastName}</span>
                </div>
              </div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-1.5">
                    {c.attendances.map((att: any) => (
                      ...
                      >
                        ...
                        <span>{att.user?.firstName} {att.user?.lastName}</span>
                        {att.user?.licenseId && (
                          <span className="font-mono text-red-400">#{att.user?.licenseId}</span>
                        )}
                      </span>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleCreateCourse} className="space-y-3 text-xs">
              ...
              <div>
                <label className="font-semibold text-slate-300">License Validity Extension (Months)</label>
                <input
                  type="number"
                  min={1}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setShowAttestModal(false)} className="text-slate-400 hover:text-white">✕</button>
            </div>
            ...
            <form onSubmit={handleAttest} className="space-y-3 text-xs">
              ...
              <div className="rounded-lg bg-slate-950 border border-slate-800 p-3 text-[11px] text-slate-400">
                ⚡ Attesting this course will automatically extend the member's license validity by <strong>{selectedCourse.validityExtensionMonths} months</strong>.
              </div>
