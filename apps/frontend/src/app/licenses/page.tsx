      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Player licenses (1 regular license per season limit, T-Cards, Women's), Coach licenses, and Referee certifications.
          </p>
        </div>
      ...
      {user?.licenseId && (
        <div className="rounded-xl border border-red-900/40 bg-gradient-to-r from-red-950/60 via-slate-900 to-slate-950 p-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Your Official Federation License ID</div>
              <div className="font-mono text-xl font-black text-white tracking-widest">{user.licenseId}</div>
            </div>
          </div>
            ...
            {licenses.map((lic) => (
              <tr key={lic.id} className="hover:bg-slate-900/40 transition">
                ...
                <td className="px-4 py-3 text-slate-300">
                  {lic.club ? lic.club.name : <span className="text-slate-500 italic">None (Tournament Card)</span>}
                </td>
                <td className="px-4 py-3 text-slate-400">
                  {lic.association?.name}
                </td>
                <td className="px-4 py-3 text-slate-400 font-mono text-[11px]">
                  {format(new Date(lic.validFrom), 'dd.MM.yy')} - {format(new Date(lic.validUntil), 'dd.MM.yy')}
                </td>
