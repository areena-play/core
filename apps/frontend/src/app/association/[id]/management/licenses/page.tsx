...
function AssociationLicensingHubContent() {
    const { user } = useAuth();
    const { t } = useI18n();
    ...
    const fetchLicenses = async () => {
        ...
        try {
            ...
            if (assocId) queryParams.associationId = assocId;
            if (typeFilter) queryParams.type = typeFilter;
            if (statusFilter) queryParams.status = statusFilter;

            const data = await api.getLicenses(queryParams);
            setLicenses(data || []);
        } catch (err) {
    ...
    useEffect(() => {
        fetchLicenses();
    }, [typeFilter, statusFilter, assocId]);
    ...
    const handleTabChange = (tab: 'directory' | 'approvals') => {
        ...
        if (tab === 'approvals') {
            urlParams.set('tab', 'approvals');
        } else {
            urlParams.delete('tab');
        }
        const prefix = assocId ? `/association/${assocId}/management/licenses` : '/management/licenses';
        router.replace(`${prefix}${urlParams.toString() ? `?${urlParams.toString()}` : ''}`);
    };
    ...
    const coursesLink = assocId
        ? `/association/${assocId}/management/licenses/refresher-courses`
        : '/management/licenses/refresher-courses';
...
export default function LicensesPage() {
    return (
        <Suspense fallback={null}>
            <AssociationLicensingHubContent />
        </Suspense>
    );
}
