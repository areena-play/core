...
export default function AssociationSettingsPage() {
    ...
    const urlAssocId = (params?.id as string) || undefined;
    ...
    const isAuthorized =
        ...
        user?.associationRoles?.some((r: any) =>
            (!topAssoc || r.associationId === topAssoc.id) && ['ADMIN', 'PRESIDENT', 'SECRETARY'].includes(r.role),
        );

    // Form state populator
    const populateFormState = (assoc: any) => {
        if (!assoc) return;
        setTopAssoc(assoc);
        setAssocName(assoc.name || '');
        setAssocShortName(assoc.shortName || '');
        setCurrentLogoUrl(assoc.logoUrl || null);
        setTemplate(assoc.licenseIdTemplate || '{regionDigit}{year2}{counter3}');
        setCounter(assoc.licenseCounter ?? 1);
        setRegionDigit(assoc.regionDigit ?? 1);

        const rules = assoc.rules || {};
        const imp = rules.impressum || {};

        setHomepageUrl(rules.homepageUrl || imp.website || '');
        setImpressumOrgName(imp.organizationName || assoc.name || '');
        ...
        if (rules.maxForeignersPerTeam !== undefined) setMaxForeignersPerTeam(Number(rules.maxForeignersPerTeam));
        if (rules.allowTCardDualRegistration !== undefined) setAllowTCardDualRegistration(Boolean(rules.allowTCardDualRegistration));
        if (rules.eloKFactor !== undefined) setEloKFactor(Number(rules.eloKFactor));

        if (Array.isArray(rules.ageSeries) && rules.ageSeries.length > 0) {
            setAgeSeries(rules.ageSeries);
        } else {
            setAgeSeries(DEFAULT_AGE_SERIES);
        }
        ...
        if (rules.requireRefereeCourseForSenior !== undefined) setRequireRefereeCourseForSenior(Boolean(rules.requireRefereeCourseForSenior));
        if (rules.refresherCourseValidityMonths !== undefined) setRefresherCourseValidityMonths(Number(rules.refresherCourseValidityMonths));
        if (rules.expiryWarningDays !== undefined) setExpiryWarningDays(Number(rules.expiryWarningDays));
        if (rules.refresherGracePeriodMonths !== undefined) setRefresherGracePeriodMonths(Number(rules.refresherGracePeriodMonths));
    };

    // Load initial data
    const loadData = async (explicitId?: string) => {
        setLoading(true);
        try {
            const targetId = explicitId || urlAssocId;
            let assoc: any = null;
            if (targetId) {
                assoc = await api.getAssociation(targetId).catch(() => null);
            }
            if (!assoc) {
                const data = await api.getAssociations().catch(() => ({ associations: [] }));
                assoc = data.associations?.find((a: any) => a.isTopLevel) || data.associations?.[0];
                if (assoc?.id) {
                    assoc = await api.getAssociation(assoc.id).catch(() => assoc);
                }
            }

            if (assoc) {
                populateFormState(assoc);

                // Load seasons & officials for this association
                const [seasonsData, officialsData] = await Promise.all([
                    api.getSeasons(assoc.id).catch(() => []),
                    api.getOfficials(assoc.id).catch(() => []),
                ]);
    ...
    const handleSaveAllSettings = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const updated = await api.updateAssociationSettings(topAssoc.id, {
                name: assocName,
