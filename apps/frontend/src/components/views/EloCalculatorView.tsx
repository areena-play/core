        ...
        <div className="w-full space-y-6 pb-16">
            <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8 shadow-sm space-y-2">
                <div className="inline-flex items-center gap-2 rounded-full bg-red-500/10 text-red-600 dark:text-red-400 px-3 py-1 text-[11px] font-bold uppercase">
                    ...
                    <span>{t('eloCalculator.officialRatingSystem', {}, 'Official Rating System')}</span>
                </div>
                <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
                    {t('eloCalculator.title', {}, 'Elo Rating Calculator')}
                </h1>
                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                    {t('eloCalculator.subtitle', {}, 'Simulate real-time ranking point exchanges based on match outcomes and k-factor weightings.')}
                </p>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                        <label className="text-xs font-bold text-slate-700 dark:text-slate-300">
                            {t('eloCalculator.player1Current', {}, 'Player 1 Current ELO')}
                        </label>
                        <input
                            type="number"
                            value={p1Elo}
                        ...
                        <span className="text-[11px] text-slate-400">
                            {t('eloCalculator.expectedWinProb', {}, 'Win Probability')}: {(expected1 * 100).toFixed(1)}%
                        </span>
                    </div>

                    <div className="space-y-2 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                        <label className="text-xs font-bold text-slate-700 dark:text-slate-300">
                            {t('eloCalculator.player2Current', {}, 'Player 2 Current ELO')}
                        </label>
                        <input
                            type="number"
                            value={p2Elo}
                        ...
                        />
                        <span className="text-[11px] text-slate-400">
                            {t('eloCalculator.expectedWinProb', {}, 'Win Probability')}: {(expected2 * 100).toFixed(1)}%
                        </span>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-700 dark:text-slate-300">
                        {t('eloCalculator.matchOutcome', {}, 'Match Outcome')}
                    </label>
                    <div className="grid grid-cols-3 gap-2">
                        {[
                            { id: 'p1', label: t('eloCalculator.player1Wins', {}, 'Player 1 Wins') },
                            { id: 'draw', label: t('eloCalculator.draw', {}, 'Draw / Tie') },
                            { id: 'p2', label: t('eloCalculator.player2Wins', {}, 'Player 2 Wins') },
                        ].map((btn) => (
                            <button
                                key={btn.id}
                                type="button"
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <div className="text-center p-4 rounded-2xl bg-slate-100 dark:bg-slate-950">
                        <span className="text-[10px] font-bold uppercase text-slate-400">
                            {t('eloCalculator.player1New', {}, 'Player 1 New Rating')}
                        </span>
                    ...
                    <div className="text-center p-4 rounded-2xl bg-slate-100 dark:bg-slate-950">
                        <span className="text-[10px] font-bold uppercase text-slate-400">
                            {t('eloCalculator.player2New', {}, 'Player 2 New Rating')}
                        </span>
                        <div className="text-2xl font-mono font-black text-slate-900 dark:text-white mt-1">
