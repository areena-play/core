...
router.post('/', authenticateToken, requireSuperAdmin, validate(createAssociationSchema), async (req: AuthRequest, res: Response, next) => {
  try {
...
router.put('/:id/settings/license-template', authenticateToken, validate(updateLicenseIdTemplateSchema), async (req: AuthRequest, res: Response, next) => {
  try {
