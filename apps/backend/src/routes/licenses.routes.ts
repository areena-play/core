...
router.post('/apply', authenticateToken, validate(applyLicenseSchema), async (req: AuthRequest, res: Response, next) => {
  try {
...
router.post('/:id/approval', authenticateToken, validate(approveLicenseSchema), async (req: AuthRequest, res: Response, next) => {
  try {
