...
export class LicenseService {
   ...
   */
  static async generateLicenseId(associationId: string): Promise<string> {
  ...
  static async applyForLicense(data: {
    ...
    if (data.type === LicenseType.PLAYER_REGULAR) {
      ...
      if (data.seasonId) {
        const existingRegular = await prisma.license.findFirst({
          where: {
            userId: data.userId,
            type: LicenseType.PLAYER_REGULAR,
            seasonId: data.seasonId,
            status: { in: [LicenseStatus.APPROVED, LicenseStatus.PENDING_CLUB, LicenseStatus.PENDING_ASSOCIATION] },
          },
        });
    ...
    if (data.type === LicenseType.PLAYER_TCARD) {
      const association = await prisma.association.findUnique({ where: { id: data.associationId } });
    ...
    // Publish event
    try {
      await redisPub.publish('areena:licenses', JSON.stringify({
  ...
  static async attestCourseAttendance(data: {
    ...
    const course = await prisma.refresherCourse.findUnique({
    ...
    if (course.instructorId !== data.instructorId) {
      ...
      const instructorUser = await prisma.user.findUnique({ where: { id: data.instructorId } });
      if (!instructorUser?.isSuperAdmin) {
        throw new Error('Only the designated course instructor or super admin can attest attendance');
      }
    }

    const targetLicenseType = course.type === 'COACH_REFRESHER' ? LicenseType.COACH : LicenseType.REFEREE;
    ...
    // Record attendance
    const attendance = await prisma.courseAttendance.upsert({
      where: {
        courseId_userId: {
